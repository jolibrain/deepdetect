...
FROM nvidia/cuda:${DD_CUDA_VERSION}-cudnn-runtime-ubuntu${DD_UBUNTU_VERSION} AS runtime

ARG DD_CUDA_MAJOR_MINOR
...
COPY --from=build /usr/local/cuda-${DD_CUDA_MAJOR_MINOR}/targets/x86_64-linux/lib/libcupti* /usr/local/cuda-${DD_CUDA_MAJOR_MINOR}/targets/x86_64-linux/lib/
