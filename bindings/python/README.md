...
LibTorch and NVIDIA CUDA runtime libraries are provided by the `torch==2.12.*`
...
`torch==2.12.*` and the Python build tools:
...
environment must provide the Python dependencies, especially `torch==2.12.*`,
`numpy`, and `Pillow`.
