...
namespace dd
{
	...
	std::vector<BaseGraph::Vertex> BaseGraph::add_outputs(std::string opname,
														  std::vector<std::string>& outputs)
	{
		std::map<std::string, BaseGraph::Vertex>::iterator it = _opIndex.find(opname);
		if (it == _opIndex.end())
			{
	...
	{
		std::map<std::string, BaseGraph::Vertex>::iterator it = _varIndex.find(varname);
  ...
  BaseGraph::compute_dims(BaseGraph::Vertex producer, BaseGraph::Vertex var)
	{
	...
	void BaseGraph::compute_dims(BaseGraph::Vertex v)
	{
		...
		auto newdims = compute_dims(producer, v);
