	...
	if (optype == "LSTM" || optype == "RNN")
	  {
		...
		if (_lstm_continuation && _rnn_has_memories[opname(v)])
		  {
			full_output = _modules[opname(v)].
			  forward<std::tuple<Tensor,std::tuple<Tensor,Tensor>>>
			  (inputsTensor[0],
			   torch::optional<std::tuple<torch::Tensor,torch::Tensor>>(_rnn_memories[opname(v)]));
		  }
		else
		  full_output = _modules[opname(v)].
			forward<std::tuple<Tensor,std::tuple<Tensor,Tensor>>>
		...
		if (_lstm_continuation)
		  {
			_rnn_memories[opname(v)] = std::get<1>(full_output);
			_rnn_has_memories[opname(v)] = true;
		  }

	  }
	else if (optype == "InnerProduct")
	  output.push_back(_modules[opname(v)].forward(inputsTensor[0]));
	else
	  throw TorchGraphException("unknown optype " + optype + " for operator " + "opname");
	return output;
  }


  void TorchGraphBackend::allocate_modules(bool force)
  {
	for (BaseGraph::Vertex v: _sortedOps)
	  {
		if (!_graph[v].alloc_needed)
		  continue;
		if (_parameters_used)
		  throw TorchGraphException("parameters reallocation necessary while there are used elsewhere. You should module.forward() / module.set_input() / module.finalize() with correct input dimensions before modules.parameters() or  module.parameters_release() if you know what you are doing");
		std::string optype = this->optype(v);
