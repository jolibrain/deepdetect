...
namespace dd
{
  ...
  public:
	...
	/**
	 * \brief allocates torch modules base on current
  ...
  protected:
	 ...
	 * @param force
	 */
	void allocate_modules(bool force = false);

	std::map<std::string,torch::nn::AnyModule> _modules; /**< torch modules, per name/id */
	std::map<std::string,torch::Tensor> _variables; /**< torch intermediate tensors / blobs  */
	...
	std::map<std::string, std::tuple<torch::Tensor, torch::Tensor>> _rnn_memories; /**< values of previsous hidden states */
	std::map<std::string, bool> _rnn_has_memories; /**< true if previsous hidden values are available */
  };
