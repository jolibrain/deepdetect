...
static std::string ocr_test_data
    = "../examples/torch/resnet18_training_torch_ocr/test.txt";
static std::string ocr_test_image
    = "../examples/torch/resnet18_training_torch_ocr/crops/8717_5.png";
...
TEST(torchapi, service_train_image_segmentation_segformer_no_db)
{
  setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8", true);
  torch::manual_seed(torch_seed);
  at::globalContext().setDeterministicCuDNN(true);
  ...
  std::string jstr
      = "{\"mllib\":\"torch\",\"description\":\"image\",\"type\":"
        ...
        + segformer_train_repo
        + "\"},\"parameters\":{\"input\":{\"connector\":\"image\","
          "\"width\":480,\"height\":480,\"db\":false,\"segmentation\":true},"
          "\"mllib\":{\"nclasses\":"
          "13,\"gpu\":true,\"segmentation\":true}}}";
  std::string joutstr = japi.jrender(japi.service_create(sname, jstr));
  ASSERT_EQ(created_str, joutstr);
...
TEST(torchapi, service_train_image_segmentation_segformer_no_db_focal)
{
  setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8", true);
  torch::manual_seed(torch_seed);
  at::globalContext().setDeterministicCuDNN(true);
  ...
  std::string jstr
      = "{\"mllib\":\"torch\",\"description\":\"image\",\"type\":"
        ...
        + "\"},\"parameters\":{\"input\":{\"connector\":\"image\","
          "\"width\":480,\"height\":480,\"db\":false,\"segmentation\":true},"
          "\"mllib\":{\"nclasses\":"
          "13,\"gpu\":true,\"segmentation\":true,\"loss\":\"focal\"}}}";
  std::string joutstr = japi.jrender(japi.service_create(sname, jstr));
  ASSERT_EQ(created_str, joutstr);
...
TEST(torchapi, service_train_image_segmentation_segformer_no_db_pad)
{
  setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8", true);
  torch::manual_seed(torch_seed);
  at::globalContext().setDeterministicCuDNN(true);
  ...
  std::string jstr
      = "{\"mllib\":\"torch\",\"description\":\"image\",\"type\":"
        ...
        + "\"},\"parameters\":{\"input\":{\"connector\":\"image\","
          ...
          "\"width\":480,\"height\":-1,\"aspect_ratio_pad\":true,\"db\":false,"
          "\"segmentation\":true},"
          "\"mllib\":{\"nclasses\":"
          "13,\"gpu\":true,\"segmentation\":true}}}";
  std::string joutstr = japi.jrender(japi.service_create(sname, jstr));
...
TEST(torchapi, service_train_images_ctc)
{
  setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8", true);
  torch::manual_seed(torch_seed);
  at::globalContext().setDeterministicCuDNN(true);
  ...
  std::string jstr
      = "{\"mllib\":\"torch\",\"description\":\"image\",\"type\":"
        ...
        + "\"},\"parameters\":{\"input\":{\"connector\":\"image\","
          "\"width\":112,\"height\":32,\"db\":true,\"ctc\":true},\"mllib\":{"
          "\"finetuning\":true,\"gpu\":true,\"timesteps\":128}}}";
  std::string joutstr = japi.jrender(japi.service_create(sname, jstr));
  ASSERT_EQ(created_str, joutstr);
  ...
  std::string jtrainstr
      = "{\"service\":\"imgserv\",\"async\":false,\"parameters\":{"
        "\"mllib\":{\"solver\":{\"iterations\":"
        + iterations_resnet50 + ",\"base_lr\":1e-4"
        + ",\"iter_size\":4,\"solver_type\":\"ADAM\",\"test_"
  ...
  ASSERT_TRUE(jd["body"]["measure"]["train_loss"].GetDouble() < 8)
      << "train loss";
  ...
  ASSERT_TRUE(fileops::file_exists(resnet18_ocr_train_repo + "checkpoint-"
                                   + iterations_resnet50 + ".ptw"));
  ASSERT_TRUE(fileops::file_exists(resnet18_ocr_train_repo + "checkpoint-"
                                   + iterations_resnet50 + ".pt"));
  for (std::string ff : lfiles)
...
TEST(torchapi, service_train_images_ctc_native)
{
  setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8", true);
  torch::manual_seed(torch_seed);
  at::globalContext().setDeterministicCuDNN(true);
  ...
  std::string jstr
      = "{\"mllib\":\"torch\",\"description\":\"image\",\"type\":"
        ...
        + "\"},\"parameters\":{\"input\":{\"connector\":\"image\","
          "\"width\":112,\"height\":32,\"db\":true,\"ctc\":true},\"mllib\":{"
          "\"template\":\"crnn\",\"gpu\":true,\"timesteps\":128}}}";
  std::string joutstr = japi.jrender(japi.service_create(sname, jstr));
  ASSERT_EQ(created_str, joutstr);
...
TEST(torchapi, service_train_images_split_regression_2dims_db_false)
{
  setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8", true);
  torch::manual_seed(torch_seed);
  at::globalContext().setDeterministicCuDNN(true);
...
TEST(torchapi, service_train_images_split_regression_2dims_db_true)
{
  setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8", true);
  torch::manual_seed(torch_seed);
  at::globalContext().setDeterministicCuDNN(true);
...
TEST(torchapi, service_train_object_detection_translation)
{
  setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8", true);
  torch::manual_seed(torch_seed);
  at::globalContext().setDeterministicCuDNN(true);
  ...
  std::string jtrainstr
      = "{\"service\":\"detectserv\",\"async\":false,\"parameters\":{"
        ...
        + ",\"iter_size\":2,\"solver_"
          ...
          "\"mirror\":true,\"rotate\":true,\"crop_size\":512,"
          "\"test_crop_samples\":10,"
          "\"cutout\":0.1,\"geometry\":{\"prob\":1.0,\"persp_horizontal\":"
          "true,\"persp_vertical\":true,\"transl_horizontal\":true,"
          "\"transl_vertical\":true,\"zoom_in\":true,\"zoom_out\":true,"
          "\"pad_mode\":\"mirrored\"},\"noise\":{\"prob\":0.01},\"distort\":{"
          "\"prob\":0.01}},\"input\":{\"seed\":12347,\"db\":true,"
...
TEST(torchapi, service_train_radam)
{
  setenv("CUBLAS_WORKSPACE_CONFIG", ":4096:8", true);
  torch::manual_seed(torch_seed);
  at::globalContext().setDeterministicCuDNN(true);
  ...
  std::string jtrainstr
      = "{\"service\":\"imgserv\",\"async\":false,\"parameters\":{"
        "\"mllib\":{\"solver\":{\"iterations\":"
        + iterations_resnet50 + ",\"base_lr\":" + torch_lr
        + ",\"iter_size\":4,\"solver_type\":\"RADAM\",\"clip\":false,"
          "\"test_interval\":"
        + iterations_resnet50
        + "},\"net\":{\"batch_size\":4},\"nclasses\":2,\"resume\":false},"
          "\"input\":{\"seed\":12345,\"db\":true,\"shuffle\":true},"
